 ...
 */
public class SpinnerDialogSubCate {
    ArrayList<SubCateModel> SubCateModel;
    ...
    int pos;
    public SpinnerDialogSubCate(Activity activity, ArrayList<SubCateModel> SubCateModel, String dialogTitle) {
        this.SubCateModel = SubCateModel;
        this.context = activity;
        this.dTitle = dialogTitle;
    }
   public SpinnerDialogSubCate(Activity activity, ArrayList<SkillsDTO> skillsDTOS, String dialogTitle, String closeTitle) {
    ...
    public void showSpinerDialog() {
        ...
        myAdapterRadio = new MyAdapterRadio(this.context, this.SubCateModel);
        ...
        close.setOnClickListener(new OnClickListener() {
            public void onClick(View v) {
                SpinnerDialogSubCate.this.alertDialog.dismiss();
            }
        });
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                ...
                String selectedID = "";
                for (int j = 0; j < SubCateModel.size(); j++) {
                    if (t.getText().toString().equalsIgnoreCase(SubCateModel.get(j).toString())) {
                        pos = j;
                        selectedID = SubCateModel.get(j).getId();
                    }
                    if (j == i) {
                        SubCateModel.get(j).setSelected(true);
                    } else {
                        SubCateModel.get(j).setSelected(false);
                    }
                }
    ...
    public class MyAdapterRadio extends BaseAdapter {
        ...
        ArrayList<SubCateModel> arrayList;
        LayoutInflater inflater;


        public MyAdapterRadio(Context context, ArrayList<SubCateModel> arrayList) {
